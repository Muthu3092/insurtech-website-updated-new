...
- ✅ **P0 fix: Lead → Customer conversion for existing emails.** In `LeadDetailPage.jsx` (`handleConvertToCustomer`) and `Leads.jsx` (`handleConvertSubmit`), when `POST /api/auth/signup` returns 400 ("Email already registered") after `POST /api/leads/{id}/convert`, we now fall back to `GET /api/crm/customers?q=<email>`, match the row by exact email, and link that existing `user_id`. The success toast becomes "Lead converted. Existing account linked as customer." and the deep link points to `/admin/customers/<userId>`. Verified end-to-end against the live backend.
- ✅ **P0 fix: Lead → Customer conversion for bulk-imported (blank-email) leads.** Excel-imported leads have `email=""`; the previous `if (email)` guard skipped signup entirely, so no user was ever created and the row never appeared in `/admin/customers`. Now we auto-generate a placeholder `lead-<leadId>@noemail.afinity.ai`, run signup, and surface a distinct toast: "Lead converted. Customer added with placeholder email — update it from the profile." Verified end-to-end: a real bulk-imported lead (`MIIVEELLEN E/L THEYEELEN`) is now visible in `/api/crm/customers` with `role=customer`.
